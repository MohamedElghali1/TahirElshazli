...
| `F5-3` `[x]` (C-1) | `GET /admin/assistants` reported an **admin** as `assigned_groups` with no groups ("0 groups"). Fixed: an admin reads `all_groups`, and one spec asserts admin and unconfigured-assistant together. | low |
