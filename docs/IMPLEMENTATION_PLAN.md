...
| `DOM-3` `[ ]` | `student_profiles` gains `school_name`, `parent_email`, `staff_notes`. **No `mode` and no conditional CHECK** — `D-4` stands (ruling R-2, 2026-09-20); five documents still say otherwise and the executor amends all five as part of this task. | — | 014 | Low |
| `DOM-4` `[ ]` | Registration approval: `users.status`; accept/reject routes; `POST /courses/:id/enroll` → staff-only. **Keep `CoursesService.enroll`.** **Takes its own migration number, not `011`** — `011_full_admin_role.sql` shipped in unit 1 with the two role CHECK widenings and nothing else, and a migration file is immutable once the ledger records it by filename. | `AUTH-1` | 014 | Medium — changes the login path |
