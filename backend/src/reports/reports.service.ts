...
export class ReportsService {
  ...
  async getSummary(courseId: string, studentId: string): Promise<ReportSummary> {
    // The gate, not a value: nothing is read off the enrollment. It still has
    // to run, and it has to run *first* - building a progress figure for a
    // course the caller does not hold would answer a question they are not
    // entitled to ask.
    ...
    const progress = await this.coursesService.getProgress(courseId, studentId);
