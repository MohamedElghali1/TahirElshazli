...
| `F5-3` `[x]` (C-1) | `GET /admin/assistants` reported an **admin** as `assigned_groups` with no groups ("0 groups"). Fixed for active accounts (`fromUser`) and, after the re-check's R-1, for pending invitations and edits too (`invite`/`update` store `all_groups` for an admin, whatever the body's `scope`). Specs cover all four cases, including an unconfigured assistant still reading as none. | low |
